export interface HPAcademyTranscriptSuccess {
  ok: true
  title: string
  url: string
  text: string
...
export interface HPAcademyTranscriptFailure {
  ok: false
  title: string
  url: string
  reason: string
...
export type HPAcademyTranscriptResult = HPAcademyTranscriptSuccess | HPAcademyTranscriptFailure

export function isHPAcademyVideoUrl(url: string): boolean {
  try {
...
export function createHPAcademyTranscriptScript(): string {
  return `(${async function captureHPAcademyTranscript(): Promise<HPAcademyTranscriptResult> {
    ...
    function textFromElements(elements: Element[]): string {
      const lines = elements
        .map((element) => cleanTranscriptLine(element.textContent ?? ''))
